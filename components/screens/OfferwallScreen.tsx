...
import { ArrowLeft, Coins, Lock, CheckCircle2, AlertCircle, ExternalLink, X } from 'lucide-react';
...
export function OfferwallScreen() {
  ...
  // Token y URL base de BitLabs con el ID del usuario actual de Firebase
  const getBitLabsUrl = () => {
    const uid = auth.currentUser?.uid ?? 'guest';
    const appToken = '3da101f5-6ff1-4ce4-9406-4fb8c6744475';
    return `https://web.bitlabs.ai/?appid=${appToken}&uid=${uid}`;
  };
            ...
            <>
              ...
              {result?.ok && !showOfferwall && (
                <div className="mt-4 flex items-center gap-2 rounded-xl bg-green-500/10 border border-green-500/30 p-3 text-green-400 text-sm">
                  ...
                  Offerwall abierto correctamente.
                </div>
              )}
              {result && !result.ok && (
              ...
              <div className="mt-6 rounded-xl bg-card border border-border p-4 text-xs space-y-2">
                ...
                <p className="text-white/40">1. Presiona &quot;Abrir Offerwall&quot; para ver las ofertas disponibles.</p>
