...
export function VersionChecker() {
  ...
  useEffect(() => {
    const checkAppVersion = async () => {
      try {
        ...
        // Enlace correcto a tu GitHub
        const res = await fetch(`https://josegabrielgar14-afk.github.io/GARRDASHPE/version.json?t=${Date.now()}`);
        if (!res.ok) return;
  ...
  const handleOpenPlayStore = () => {
    window.open('https://play.google.com/store/apps/details?id=com.garrcraft.garrdash', '_blank');
  };
